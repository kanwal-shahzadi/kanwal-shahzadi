# 👋 Hi, I'm Kanwal Shahzadi

### 📌 Introduction
I am **Kanwal Shahzadi**, a **PhD scholar in Mathematics** with a strong academic and teaching background.  
I have completed my **MS in Mathematics (2024)** and **BS in Mathematics (2020)**.  
My research interests include **numerical analysis, differential equations, and mathematical modeling**.  
Currently, I am **learning Python** to strengthen my PhD research and computational skills.

---

## 🎓 Academic Status
- **PhD in Mathematics** (Ongoing)
- **MS Mathematics** – GC Women University Sialkot (2024)
- **BS Mathematics** – GC Women University Sialkot (2020)
- 

## 🛠️ Skills & Tools

### Programming & Software
- 🐍 Python (Beginner – Learning)
- MATLAB
- Mathematica
- Latex
- MS Office (Word, Excel, PowerPoint)

### Mathematical Expertise
- Numerical Methods
- Differential Equations
- Mathematical Modeling
- Analytical & Numerical Solution Techniques
- Research-Oriented Computations



## 📂 Projects

### 🔢 Python Learning Projects
- Python basics (functions, loops, conditions)
- Mathematical computations using Python  
📌 *Repositories will be added as learning progresses*

### 📊 Numerical Methods (MATLAB)
- Heat Equation (Finite Difference Method)
- Tridiagonal & Pentadiagonal Matrix Problems
- Error and convergence analysis

### 📐 Symbolic & Analytical Work
- Equation derivations using Mathematica
- Research-based mathematical computations


## 👩‍🏫 Teaching Experience
- Lecturer in Mathematics
- Mathematics Teacher (Secondary Level)
- **2+ years of teaching experience**


## 🔬 Research Interests
- Analytical and Numerical Solutions of Differential Equations
- Numerical Analysis
- Computational Mathematics
- Mathematical Modeling



## 📬 Contact Information
- 📧 Email: **kanwal.shahzadi16025@gmail.com**
- 💼 LinkedIn: [Kanwal Shahzadi](https://www.linkedin.com/in/kanwal-shahzadi-99753127b)
- 💻 GitHub: https://github.com/kanwal-shahzadi



## 🌟 Fun / Optional
- 📘 Passionate about Mathematics & Research
- 💡 Learning Python for PhD-level computations
- 🔬 Interested in bridging theory with computation

---

⭐ *Feel free to explore my repositories and follow my research & learning journey!*

